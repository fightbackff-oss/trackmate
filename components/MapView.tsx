...
const createCustomMarkerHtml = (user: User, isCurrentUser: boolean) => {
  ...
  // Visual indicator for Ghost Mode (not sharing)
  const borderClass = isCurrentUser 
    ? (user.isSharing ? 'border-blue-500' : 'border-slate-400 dashed') 
    : 'border-white';
    
  const opacityClass = (isCurrentUser && !user.isSharing) ? 'opacity-70' : 'opacity-100';
  
  return `
    <div class="relative w-12 h-12 ${opacityClass}">
      <div class="w-12 h-12 rounded-full border-[3px] ${borderClass} shadow-md overflow-hidden bg-gray-200 box-border">
...
const MapController = ({ 
  ...
  useEffect(() => {
      // Ensure lat/lng are numbers before using
      if(userLocation && typeof userLocation.lat === 'number' && typeof userLocation.lng === 'number' && !map.getCenter()) {
...
const MapView: React.FC<MapViewProps> = ({ 
  ...
  // Safety check for userLocation
  const hasValidUserLocation = userLocation && typeof userLocation.lat === 'number' && typeof userLocation.lng === 'number';
  ...
  return (
    <div className="w-full h-full relative bg-[#0B1120]">
      ...
      >
        ...
        {/* Current User Marker - Always show to self, even if not sharing */}
        ...
        {friends.map((friend) => {
          ...
          // STRICT CHECK: lat/lng must be numbers
          if (typeof friend.location.lat !== 'number' || typeof friend.location.lng !== 'number') return null;
