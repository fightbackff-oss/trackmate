...
import { 
  Menu, Search, Layers, Locate, Plus, Minus, AlertTriangle, Sparkles
} from 'lucide-react';
...
const FloatingMapControls = ({ 
  ...
  <div className="absolute bottom-32 right-6 z-[1000] flex flex-col space-y-4 pointer-events-none">
    <button onClick={onToggleAI} className={`w-12 h-12 rounded-[20px] flex items-center justify-center shadow-lg pointer-events-auto active:scale-95 transition-all ${isAIOpen ? 'bg-[#a3e635] text-black shadow-[#a3e635]/30' : 'glass-panel bg-white/80 dark:bg-transparent text-slate-600 dark:text-gray-300 hover:text-[#a3e635]'}`}>
      <Sparkles size={22} className={isAIOpen ? 'animate-pulse' : ''} />
    </button>
...
const App: React.FC = () => {
  ...
  const initUserProfile = async (authUser: any) => {
    try {
      ...
      if (!profile) {
        ...
        await supabase.from('users').upsert(newProfile); // Upsert is safer here
  ...
  // 4️⃣ GET FRIEND LIST (As per requirements)
  const fetchFriends = async (userId: string) => {
    try {
      // RLS policies will automatically hide non-friends or friends who disabled tracking from the live_locations join
  ...
  // 2️⃣ GET RECEIVED REQUESTS (As per requirements)
  const fetchFriendRequests = async (userId: string) => {
    try {
      const { data, error } = await supabase
        .from("friend_requests")
        .select("*, sender:users!requester_id(*)") // Joined for UI
  ...
  // --- 3. LOCATION TRACKING (5️⃣ UPDATE LIVE LOCATION) ---
  const startTracking = useCallback(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      async (position) => {
        ...
        const now = Date.now();
        // Throttle updates: Only update DB every 10 seconds to save resources
        if (currentUser?.isSharing && now - lastLocationUpdate.current > 10000) {
    ...
    try {
      ...
      if(users && users.length > 0) {
        ...
        const { error } = await supabase.from('friend_requests').insert({ 
            ...
            status: 'pending' // status is optional as default is pending, but adding for clarity
        });
  ...
  const handleAcceptRequest = async (request: FriendRequest) => {
      ...
      try {
          ...
          // 2. Insert into friends table TWICE (Bidirectional Friendship)
          // Insert Requester -> Receiver AND Receiver -> Requester
          ...
          // Refresh lists
          fetchFriendRequests(receiverId);
          fetchFriends(receiverId);
      } catch (e: any) { 
